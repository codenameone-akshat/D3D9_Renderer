...
PS_OUTPUT RenderPS(in VS_OUTPUT psInput)
{
    ...
    float3 reflectedLightDir = normalize(reflect(-lightDir, vWorldNormal)); //get the reflected ray for the incident light

    float RoV = pow(saturate(dot(reflectedLightDir, viewDir)), g_specIntensity);

   /* float3 halfVec = normalize(float3(viewDir + g_dirLightDir.xyz));
    float NoH = saturate(dot(vWorldNormal, halfVec));
    */

    float4 specular = g_specularLightColor * RoV;// pow(NoH, g_specIntensity * 10);
